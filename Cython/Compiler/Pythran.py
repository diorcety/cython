...
try:
    import pythran
    pythran_version = version_regex.search(pythran.__version__)
    if not pythran_version:
        raise InvalidVersion("Invalid pythran version: '{0}'".format(pythran.__version__))
    pythran_release = tuple(int(i) for i in pythran_version.group("release").split("."))
    pythran_is_pre_0_9 = pythran_release[0:2] < (0, 9)
    pythran_is_pre_0_9_6 = pythran_release[0:3] < (0, 9, 6)
